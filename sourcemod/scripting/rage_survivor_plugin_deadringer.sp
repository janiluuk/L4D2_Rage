...
public Action DeadRinger_Command(int client, int args) {
...
Action DeadRinger_Force_Command(int client, int args)
{
	if (args != 1)
	{
...
Action HitMyself_Command(int client, int args) {
	if (!IsValidClient(client)) return Plugin_Handled;
	if (args < 1 || args > 1)
	{
...
void Hook_OnTakeDamagePost(int client, int attacker, int inflictor, float damage, int damageType, int weapon, const float damageForce[3], const float damagePosition[3])
{
...
void DeadRingerCloak(int client, int attacker, int inflictor, int damageType, int weapon)
{
	...
	PrintHintText(client, "Cloak activated.");
}
...
void DeadRingerUncloak(int client, bool killtimer = true)
{
	...
	PrintHintText(client, "You uncloaked.");
