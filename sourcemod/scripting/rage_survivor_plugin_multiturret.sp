...
#define EXTENDED_COLOR_TAG_NONE      0 		
#define EXTENDED_COLOR_TAG_TEAM      1 		
...
#define PARTICLE_GAS_EXPLOTION 				"gas_explosion_pump" 				
#define PARTICLE_MOLOTOV_EXPLOTION 			"molotov_explosion" 				
#define PARTICLE_MOLOTOV_EXPLOTION_BRANCH 	"molotov_explosion_child_burst" 	
...
#define SOUND_IMPACT_FLESH		"physics/flesh/flesh_impact_bullet1.wav" 		
...
#define MODEL_MINIGUN_GATLING 	"models/w_models/weapons/w_minigun.mdl" 		
#define MODEL_MINIGUN_50CAL 	"models/w_models/weapons/50cal.mdl" 			
#define MODEL_CRATE				"models/props_junk/explosive_box001.mdl" 		
#define MODEL_GASCAN			"models/props_junk/gascan001a.mdl" 				
#define MODEL_ANOMALY_LASER_L4D1 	"materials/sprites/physbeam.vmt" 			
#define MODEL_ANOMALY_LASER_L4D2 	"materials/sprites/laserbeam.vmt" 			
#define MODEL_MUZZLEFLASH 			"sprites/muzzleflash4.vmt" 					
/************************************************************************************************************************/
...
bool FreezedPlayer[MAXPLAYERS+1];
bool VomitedPlayer[MAXPLAYERS+1];
float ScanTime = 0.0;
...
static ConVar hCvar_Machine_SingleTurretMode;
...
static int iCvar_MachineLimitGatling;
static int iCvar_MachineEnableExplosion;

static bool bCvar_Machine_Enabled;
static bool bCvar_Machine_SingleTurretMode;

static bool bCvar_MachineAmmoReload;
static bool bCvar_MachineAmmoReloadGatling;
...
{	
	...
	if(!PrepSDKCall_SetSignature(SDKLibrary_Server, bLeft4DeadTwo ? "\x2A\x2A\x2A\x2A\x2A\x2A\x83\x2A\x2A\x83\x2A\x2A\x55\x8B\x2A\x2A\x89\x2A\x2A\x2A\x8B\x2A\x83\x2A\x2A\x56\x57\x8B\x2A\xE8\x2A\x2A\x2A\x2A\x84\x2A\x0F\x85\x2A\x2A\x2A\x2A\x8B\x2A\x8B" : "\x83\x2A\x2A\x2A\x8B\x2A\xE8\x2A\x2A\x2A\x2A\x84\x2A\x0F\x85\x2A\x2A\x2A\x2A\x8B\x2A\x8B", bLeft4DeadTwo ? 45 : 22)) // Cargar En Windows
		PrepSDKCall_SetSignature(SDKLibrary_Server, "@_ZN13CTerrorPlayer11OnStaggeredEP11CBaseEntityPK6Vector", 0);
	...
	hCvar_MachineGlowRange 			= CreateConVar("l4d_machine_glow_range", 			"3.0",		"Sets the amount of glow the turret produces", FCVAR_NOTIFY, true, 0.0, true, 100.0);
 	hCvar_MachineBetrayChance 		= CreateConVar("l4d_machine_betray_chance", 		"10.0",		"Sets the probability of being betrayed by machine guns", FCVAR_NOTIFY, true, 0.0, true, 100.0);
...
//	hCvar_ = CreateConVar("l4d_machine_", "1", "", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	...
	hCvar_Machine_SingleTurretMode.AddChangeHook(ConVarChange);	
	...
	hCvar_MachineAmmoReloadGatling.AddChangeHook(ConVarChange);	
	hCvar_MachineAllowCarry.AddChangeHook(ConVarChange);
	hCvar_MachineAllowCarryGatling.AddChangeHook(ConVarChange);	
...
/**
 * @note Current game mode is allowed 
 *
 * @return true
 */
...
public Action Event_PlayerUses(Event hEvent, const char[] sName, bool bDontBroadcast)
{
	...
	if(StrContains(sClassName, "ammo") >= 0)
	{
		...
		if(Index >= 0)
		{
			if (isGatlingGun(Index) && bCvar_MachineAmmoReloadGatling == false) {
				return Plugin_Continue;
			}
			else if (is50calGun(Index)&& bCvar_MachineAmmoReload == false) {
				return Plugin_Continue;
			}
...
public int GetMinigun(int client)
{
...
public Action CMD_SpawnMachine(int client, int args)
{
	...
	{		
		...
		else if(args == 2)
		{
			...
			if(StrEqual(sArg, "1 basic") && bBasicAllowed) 
			{
				CreateMachine(client, MACHINE_MINI, NULL);
			}
			else if(StrEqual(sArg, "2 basic") && bBasicAllowed) 
			{
				CreateMachine(client, MACHINE_50CAL, NULL);
			}
			else if(StrEqual(sArg, "1 flame") && bFlameAllowed) 
			{
			...
			else if((StrContains(sArg, "1 ") >= 0 || StrContains(sArg, "2 ") >= 0) && (!bBasicAllowed ||  !bFlameAllowed || !bLaserAllowed || !bTeslaAllowed || !bFreezeAllowed || !bNauseatingAllowed))
			{
...
void PrintUserSelection(int client, const int iMachineGunModel, const int iSpecialType = NULL) 
{
...
public Handle ShowEmptinessPanel(int client) 
{
	static char sTitle[128];
	Format(sTitle, sizeof sTitle, "%T", "Main Menu Title", client);
...
public int PanelHandler(Handle hPanel, MenuAction hAction, int iParam1, int iParam2)
{
...
public int MenuHandler_BasicMachineGuns(Menu hMenu, MenuAction hAction, int Param1, int Param2)
{
	switch(hAction)
	{
		...
		case MenuAction_Select:
		{
			...
			else if(StrEqual(sInfo, "GATLING_FIRE", false))
			{				
...
void DealTurretDamage(int victim, int attacker = 0, int iTeam, int iSpecialType = NULL, const float vStartingPos[3], const float vEndPos[3])
{
	...
	int DMG_EXPLOSIVE = (DMG_HEADSHOT - DMG_PLASMA - DMG_PHYSGUN - DMG_BLAST - DMG_BULLET) * -1; 	
...
//	if(IsTank(victim) && !bLeft4DeadTwo)
	...
	else if(iCvar_MachineAmmoType == 1) 	DMG_TYPE = bLeft4DeadTwo ? DMG_INCENDIARY : DMG_BURN; 	
	else if(iCvar_MachineAmmoType == 2) 	DMG_TYPE = DMG_EXPLOSIVETYPE; 							
	
	if(iSpecialType == TYPE_FLAME) 		DMG_TYPE = DMG_BURN;
	if(iSpecialType == TYPE_LASER) 		DMG_TYPE = IsInfected(victim) ? DMG_BLAST : DMG_ENERGYBEAM; 
	...
	{
		fDamage = GetDamageDistance(vStartingPos, vEndPos, fDamageIndex);
		DMG_TYPE = GetDistanceDamageType(vStartingPos, vEndPos); 			
	}
...
bool IsSingleTurretMode()
{
	if(bCvar_Machine_SingleTurretMode)
		return true;
...
int SpawnMiniGun(int client, int index, int iMachineGunModel, int iSpecialType = NULL)
{
	...
	else if(iSpecialType == TYPE_TESLA)
	{
		...
		CreateLight(iEntity, "0 0 255 255"); 
		bAllowSound[iEntity] = true;
	}
	else if(iSpecialType == TYPE_FREEZE)
	{
	...
	if (bCvar_MachineBlocking != true)
		SetEntData(iEntity, g_CollisionOffset, 1, 4, true);
...
void Carrying(int index, float interval)
{
	...
	if(Button & IN_USE)
	{
		...
		//			PrintHintText(client, "Now you can't drop your machine gun"); 
			return;
		}
	...
	if(index >= 0)
	{
		...
		if(attacker > 0 && attacker <= MaxClients)
		{
			if(IsClientInGame(attacker))
			{
				...
				else if(GetClientTeam(attacker) == TEAM_SURVIVOR && IsMeleeAttack(attacker)) 
				{
...
//			GunState[index] = State_Scan;
		...
		float fMachineHealth = isGatlingGun(index) ? fCvar_MachineHealthGatling : fCvar_MachineHealth;

		if(bPrint)
			PrintHintText(attacker, "%t", "Machine Gun Power", RoundFloat(GunHealth[index]), RoundFloat(fMachineHealth));
	}
}
...
{	
	...
	if(iIndexEntity >= 0)
	{
		if(GunUser[iIndexEntity] != client && GunState[iIndexEntity] == State_Carry) 
		{
...
{
	if(GetEntPropEnt(client, Prop_Send, "m_hUseEntity"))
	{
		if(MachineGunerTime[client] + 1.0 < GetEngineTime())
		{
			...
			CreateTimer(0.1, UserPushTimer, hPack, TIMER_FLAG_NO_MAPCHANGE); 
		}
	}
}
...
//	SetEntPropEnt(client, Prop_Send, "m_usingMountedWeapon", 0); 
...
void ScanAndShootEnemy(int index, float time, float interval)
{
	...
	if(bExistingEntity == false || Broken[index])
	{
		if(IsValidClient(client))
		{
			PrintHintText(client, "%t", "Damaged Machine Gun");
...
//	Broken[index] = true;
	...
	if(GunType[index] == MACHINE_MINI) 		
		ScaleVector(vTemp, 20.0);
	else if(GunType[index] == MACHINE_50CAL) 
		...
		if(newenemy > 0 && FloatAbs(diff1) < 40.0)
		{
			if(time >= GunFireTime[index] && GunAmmo[index] > 0)
			{
				...
				if(AmmoIndicator[index] >= ammoCount / 20.0)
				{
					...
					if(IsValidClient(client)) 
						//CustomPrintToChat(client, "%t", "Current Ammo", GunAmmo[index], RoundFloat(GunAmmo[index] * 100.0 / ammoCount));
						PrintCenterText(client, "Turret ammo: [%d/%d%%]", GunAmmo[index], RoundFloat(GunAmmo[index] * 100.0 / ammoCount));
				}
	...
	if(TR_DidHit(hTrace))
	{
		...
		if(enemy > 0)
		{
			...
			if(MachineGunTypes[iEntity] == TYPE_FLAME)
			{
				...
				CreateLight(iEntity, "255 30 0 255", 2, true); 
			}
			else if(MachineGunTypes[iEntity] == TYPE_LASER)
			{
				CreateLaser(vMachinePosition, vTargetPosition);
//				CreateBeam("LaserBeam0", "LaserBeam_end0", vMachinePosition, vTargetPosition, client, "50", "0 255 0 255", 0.1);
			...
			{
				MakeEnvSteam(iEntity, vMachinePosition, vAng, "-1 -1 -1 255"); 
...
bool IsValidEnemy(int enemy, int iTeam)
{
	bool bIndex = false;
	if(enemy <= 0) 
		return bIndex;
...
int GetActiveMelee(int client)
{
	...
	else if(StrEqual(sClassName, "weapon_chainsaw")) 
		return 50;

	return NULL;
}

int GetMeleeWeaponDamage(int entity) 
{	
	...
	{		
		...
		if(StrEqual(sClassName, "weapon_melee"))
		{
			...
			if(StrContains(sModel, "fireaxe") >= 0) 			return 100; 
			else if(StrContains(sModel, "v_bat") >= 0) 			return 50; 
			else if(StrContains(sModel, "crowbar") >= 0) 		return 35; 
			else if(StrContains(sModel, "electric_guitar") >=0) return 35; 
			else if(StrContains(sModel, "cricket_bat") >= 0) 	return 50; 
			else if(StrContains(sModel, "frying_pan") >= 0) 	return 15; 
			else if(StrContains(sModel, "golfclub") >= 0) 		return 35; 
			else if(StrContains(sModel, "machete") >= 0) 		return 50; 
			else if(StrContains(sModel, "katana") >= 0) 		return 50;
			else if(StrContains(sModel, "tonfa") >= 0) 			return 15; 
			else if(StrContains(sModel, "riotshield") >= 0) 	return 15; 
			else if(StrContains(sModel, "knife") >= 0) 			return 25;
			else if(StrContains(sModel, "v_shovel") >= 0) 		return 45; 
			else if(StrContains(sModel, "v_pitchfork") >= 0) 	return 40; 
		}
	}
...
int is50calGun(int index)
{
	if(GunType[index] == MACHINE_50CAL) {
...
int countGatlingGuns(int client)
{
	...
	for(int i = 0; i < MachineCount; i++)
	{
		if(GunOwner[i] == client && isGatlingGun(i))
		{
				count++;				
		}	
	}
...
int count50calGuns(int client)
{
	...
	for(int i = 0; i < MachineCount; i++)
	{
		if(GunOwner[i] == client && is50calGun(i))
		{
				count++;				
		}	
	}
...
bool hasBuildsLeftFor50cal(int client) {
	if(! IsClientPlaying(client)) return false;
	return (count50calGuns(client) < iCvar_MachineLimit);
}
...
{
	float fLaserLife = 0.38;  							
	float fLaserWidth = bLeft4DeadTwo ? 1.0 : 4.0; 		
	int iLaserColor[4];
	iLaserColor = view_as<int>({ 255, 0, 0, 255 }); 	
//	iLaserColor = view_as<int>({ 100, 0, 150, 255 }); 	
...
//	Format(sRenderColor, sizeof sRenderColor, "%d %d %d", GetRandomInt(0 , 255), GetRandomInt(0 , 255), GetRandomInt(0 , 255));
	...
	char sLife[32];
	FloatToString(fTimeDuration, sLife, sizeof sLife); 
...
{	
	...
	InputKill(iTarget, 1.0); 	
	InputKill(particle, 0.8);
	
	PlaySound(particle, sArraySoundsZap[GetRandomInt(0, sizeof(sArraySoundsZap) - 1)]); 
}
...
stock int CreateExplosionCompat(const float vPos[3], const float vAng[3], int iDamage = 0, int iRadius = 500, int iFlags = 0)
{
	...
	DispatchKeyValue(iPhysExplosion, "magnitude", sDamage);
	...
	for(int i = 1; i <= MaxClients; i++)
	{
		...
		GetClientAbsOrigin(i, vEnd);
		if(GetVectorDistance(vPos, vEnd) <= fRadius)  
...
stock void StartGlowing(int entity, int TeamIndex)
{
	...
	sColor = GetColorIndex(GetMachineGunColor(entity));
	
	if(TeamIndex == 3)
		sColor = GetColorIndex(1); 
...
stock char[] GetColorIndex(int iColorType)
{
	...
	switch(iColorType)
	{
		...
		case 13:Format(sColor, sizeof sColor, "0 100 0 255"); 	// Dark Green
...
{	
	...
	static char sColor[16];
	sColor = GetColorIndex(GetMachineGunColor(iEntity));
...
stock void CustomPrintToChat(int client, const char[] sBuffer, any ...)
{
	...
	Format(sTemp, sizeof sTemp, "\x01%s", sBuffer); 
	VFormat(sMessage, sizeof sMessage, sTemp, 3); 	
