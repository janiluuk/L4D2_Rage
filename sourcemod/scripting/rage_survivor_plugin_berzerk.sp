...
public OnCommonHurt(Handle:hEvent, String:event_name[], bool:dontBroadcast)
{
...
MagStart (iEntid, client)
{
	...
	//we change the playback rate of the gun, just so the player canv "see" the gun reloading faster
...
Announce(client)
{
	...
	switch(GetConVarInt(g_cvarAnnounceType))
	{
		case 1:
		{
			PrintToChat(client, "\x04Now you're pissed! Push MIDDLE BUTTON to go berserk!");
		}
		case 2:
		{
			PrintHintText(client, "You're on a kill spree! Push MIDDLE BUTTON to go berserk!");
		}
		case 3:
		{
			PrintCenterText(client, "Berserker mode is ready! Press MIDDLE BUTTON to activate it!");
		}
		case 4:
		{
			IntToString(GetClientUserId(client)+25, sUser, sizeof(sUser));
			Format(sMessage, sizeof(sMessage), "Now you're pissed! Push MIDDLE BUTTON to go berserk!");
