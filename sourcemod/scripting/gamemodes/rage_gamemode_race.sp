...
Handle cvar_on; //handle for the cvar sm_racemod_on
Handle snareTimer[MAXPLAYERS + 1]; //timers for calling delayPlayer
int snareCount[MAXPLAYERS + 1]; //number of times delayPlayer was called for a survivor
int snarerId[MAXPLAYERS + 1]; //ids of attacking special inf
int finishId[4]; //ids of people who reached safe room
int place; //number of survivors that made it to the saferoom
int award[4]; //award scores
char clientId[4][25]; //steam ids of clients
int score[4]; //keep track of clients current scores
bool incap[4]; //keep track of who is incapped.
int stillRacing; //number of players who have not made it to the safe room and have not been incapacitated
int mapNum; //the number that points to the correct array of saferoom position coords.
char nameFromModel[10]; //no idea how to use references.
Handle infoTimer = INVALID_HANDLE; //the handle for the timer that spits out random facts to players
int msgNum; //keep track of the info message to report
int secondsToGo; //keep track of how many seconds till racing starts.
int finaleTips; //this is not 0 if the level is a finale
...
int safeRoomArea[26][6];

bool isEnabled = false;

public Plugin myinfo = {
	name = "[Rage] L4D Race Mod",
	author = "KrazyKyleP & Yani",
	description = "Allows 4 player campaigns to become a race to the finish.",
...
public void OnPluginStart()
{
	...
	cvar_on = CreateConVar("rage_racemod_on", "0", "Is the racemod plugin turned on?", FCVAR_PLUGIN|FCVAR_SPONLY);
	HookConVarChange(cvar_on, OnRaceModChanged);
	...
	AutoExecConfig(true, "rage_gamemode_race");
}
...
public void OnRaceModChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
	bool shouldEnable = GetConVarInt(cvar_on) > 0;
	
	if(isEnabled == shouldEnable) return;
	
	if(shouldEnable) {
		isEnabled = true;
		PrintToChatAll("\x04[RaceMod]\x01 Race mode enabled!");
	} else {
		ResetRaceMod();
		isEnabled = false;
		PrintToChatAll("\x04[RaceMod]\x01 Race mode disabled!");
	}
}
...
public Action chatCMD(int client, int args)
{
...
public Action forceStartRace(int client, int args)
{
	secondsToGo = 1;
	return Plugin_Handled;
}

public Action Event_PourCompleted(Event event, const char[] name, bool dontBroadcast)
{
	if(GetConVarInt(cvar_on) > 0)
	{
		...
		GetClientModel(user, modelName, sizeof(modelName)); 
		...
		for(int i=0; i<4; i++)
		{
			if(strcmp(nameFromModel, clientId[i]) == 0)
			{
				...
				PrintToChatAll("\x04[RaceMod]\x01 %s earned 2 points for pouring a gascan!", plName);
			}	
		}
	}
...
public Action Event_PlayerPickedUp(Event event, const char[] name, bool dontBroadcast)
{
	if(GetConVarInt(cvar_on) > 0)
	{
		...
		for(int i=0; i<4; i++){
			if(strcmp(nameFromModel, clientId[i]) == 0){
				...
				#if DEBUG
				PrintToChatAll("\x04[RaceMod]\x01 %s was picked up.", nameFromModel);
				#endif	
				stillRacing++;
			}	
		}
...
public Action Event_roundStart(Event event, const char[] name, bool dontBroadcast)
{
	if(GetConVarInt(cvar_on) > 0)
	{
		...
		char mapAbbriv[5];
		strcopy(mapAbbriv, sizeof(mapAbbriv), charArr[0]);
...
public Action setup(Handle timer, any client) 
{
	if(GetConVarInt(cvar_on) > 0)
	{
		...
		char fourthChar[2];
		strcopy(fourthChar, sizeof(fourthChar), charArr[0]);
		...
		{
			#if DEBUG
			PrintToChatAll("\x04[RaceMod]\x01 Detected no ids, resetting vars.");
			#endif
			...
			int k = 0;
			char modelName[40];
			for(int i=1; i<=MaxClients; i++) //find everyone's client id.
			{	
				if(IsClientInGame(i))
				{
					...
					{ 
						...
						#if DEBUG
						PrintToChatAll("\x04[RaceMod]\x01 stored auth string: %s.", clientId[k]);
						#endif
						k++;
					}	
				}
...
public Action Event_PlayerGrabbed(Event event, const char[] name, bool dontBroadcast) //set up events to happen when player is grabbed by special inf.
{
	if(GetConVarInt(cvar_on) > 0)
	{
		...
		#if DEBUG
		PrintToChatAll("\x04[RaceMod]\x01 player grabbed");
		#endif
	
		if(snareTimer[victimClient] == INVALID_HANDLE)
		{
			#if DEBUG
			PrintToChatAll("\x04[RaceMod]\x01 started a timer");
			#endif
...
public Action delayPlayer(Handle timer, any client) //delay the player by 5 seconds after grabbed by special inf.
{
	...
	{
		#if DEBUG
		PrintToChatAll("\x04[RaceMod]\x01 turn on god mode");
		#endif
	...
	if(snareCount[client] >= 10){ //victim can now go free.
		#if DEBUG
		PrintToChatAll("\x04[RaceMod]\x01 kill timer/turn off god");
		#endif
...
public Action Event_PlayerHurt(Event event, const char[] name, bool dontBroadcast) //turn off friendly fire
{
	if(GetConVarInt(cvar_on) > 0)
	{
		...
		#if DEBUG
		PrintToChatAll("\x04[RaceMod]\x01 weapon: %s", weapon);
		#endif
		...
		if(strcmp(weapon, "inferno") != 0 && strcmp(weapon, "fire_cracker_blast") != 0 && attacker != 0)
		{
			if(IsClientInGame(victim) && IsClientInGame(attacker))
			{
				...
				{
					SetEntityHealth(victim,(event.GetInt("dmg_health")+ event.GetInt("health"))); //set friend's health back to what is was before FF.
					#if DEBUG
					PrintToChatAll("\x04[RaceMod]\x01 NO FF for you! vic: %i, att: %i", GetClientTeam(victim), GetClientTeam(attacker));
					char modelName[40];
					GetClientModel(victim, modelName, sizeof(modelName)); 
					PrintToChatAll("\x04[RaceMod]\x01 vic model: %s ", modelName);
					#endif
				}			
			}	
		}
...
public Action Event_PlayerIncap(Event event, const char[] name, bool dontBroadcast) //if player was incapped set vars assuming he wont be picked up.
{
	if(GetConVarInt(cvar_on) > 0)
	{
		int victimId = event.GetInt("userid");
		int player = GetClientOfUserId(victimId);
		...
		for(int i=0; i<4; i++){
			if(strcmp(nameFromModel, clientId[i]) == 0){
				...
				stillRacing--;
				#if DEBUG
				PrintToChatAll("\x04[RaceMod]\x01 %s was incapped.", nameFromModel);
				#endif	
			}	
		}
	...
	if(GetConVarInt(cvar_on) > 0)
	{
		...
		if(strcmp(victimName, "Tank") == 0) //if a tank died
		{
			if(IsClientInGame(killer))
			{
				...
				{
					GetClientModel(killer, modelName, sizeof(modelName)); 
					model2name(modelName); //this stores the name in nameFromModel
					for(int i=0; i<4; i++)
					{
						if(strcmp(nameFromModel, clientId[i]) == 0)
						{
							...
							score[i] += 5;
							PrintToChatAll("\x04[RaceMod]\x01 %s earned 5 points for killing a tank!", plName);
						}	
					}
				}
			}
				...
				if(GetClientTeam(killer) == 2) //client is a survivor
				{
					...
					for(int i=0; i<4; i++)
					{
						if(strcmp(nameFromModel, clientId[i]) == 0)
						{
							GetClientName(killer, plName, sizeof(plName)); 
							score[i] += 3;
							PrintToChatAll("\x04[RaceMod]\x01 %s earned 3 points for killing a witch!", plName);
						}	
					}
				}
			}
			...
			if(IsClientInGame(killer)) //you only get a chance of getting an item from a special if you kill it without it snaring you.
			{
				if(GetClientTeam(killer) == 2) //client is a survivor
				{
					...
					if(randInt == 0){
						...
						PrintToChat(killer,"\x04[RaceMod]\x01 That special you killed dropped something. Pick it up!");
					}else if(randInt == 1){
					...
					#if DEBUG
					PrintToChatAll("\x04[RaceMod]\x01 Give item for special kill? rand: %i", randInt);
					#endif
				}		
			}
		}
			...
			if(GetClientTeam(player) == 2) //client is a survivor
			{
				...
				for(int i=0; i<4; i++){
					if(strcmp(nameFromModel, clientId[i]) == 0){
						...
						#if DEBUG
						PrintToChatAll("\x04[RaceMod]\x01 %s was killed.", nameFromModel);
						#endif	
					}	
				}
			}
...
public Action Event_PlayerWin(Event event, const char[] name, bool dontBroadcast) //give the player a score.
{
	if(GetConVarInt(cvar_on) > 0)
	{
		...
		if(player != 0) //if the player is not an npc
		{
			...
			{
				float vec[3];
				GetClientAbsOrigin(player, vec);
				int posX = RoundToCeil(vec[0]);
				int posY = RoundToCeil(vec[1]);
				int posZ = RoundToCeil(vec[2]);
				...
				{
					#if DEBUG
					PrintToChatAll("\x04[RaceMod]\x01 A player has won.");
					#endif	
					...
					for(int i=0; i<4; i++)
					{
						if(finishId[i] == player)
						{
							...
							#if	DEBUG
							PrintToChatAll("\x04[RaceMod]\x01 player has already finished.");
							#endif	
						}
					}
					...
					if(awardPoints)
					{
						...
						PrintToChatAll("\x04[RaceMod]\x01 %s finished. %i points awarded. %i points total.", plName, pointsToAdd, totalPoints);
						place++;
						stillRacing--;
						if(stillRacing < 1)
						{
					...
					#if DEBUG
					PrintToChatAll("\x04[RaceMod]\x01 id: %i 0: %i 1: %i 2: %i 3: %i.", player, finishId[0], finishId[1], finishId[2], finishId[3]);
					#endif
				}
			}
		}
		#if DEBUG
		PrintToChatAll("\x04[RaceMod]\x01 Event_PlayerWin id: %i", player);
		#endif	
	}
...
public Action Event_FinaleOver(Event event, const char[] name, bool dontBroadcast) //the finale round is over, award survivors final points and report the winners.
{
	if(GetConVarInt(cvar_on) > 0)
	{
		PrintToChatAll("\x04[RaceMod]\x01 Survivors win 10 points!.");
		char modelName[40];
...
public void model2name(const char[] model) //convert a model name to just a character name.
{ 
	if(strcmp(model, "models/survivors/survivor_coach.mdl") == 0){
		strcopy(nameFromModel, sizeof(nameFromModel), "Coach");
	}else if(strcmp(model, "models/survivors/survivor_producer.mdl") == 0){
		strcopy(nameFromModel, sizeof(nameFromModel), "Rochelle");
	}else if(strcmp(model, "models/survivors/survivor_mechanic.mdl") == 0){
		strcopy(nameFromModel, sizeof(nameFromModel), "Ellis");
	}else if(strcmp(model, "models/survivors/survivor_gambler.mdl") == 0){
		strcopy(nameFromModel, sizeof(nameFromModel), "Nick");
	}else{
		strcopy(nameFromModel, sizeof(nameFromModel), "NULL");
	}
}

public void reportScores(int client) //report the scores in order. if client is not 0 then a player asked to have it printed to them
{
	#if DEBUG
	PrintToChatAll("\x04[RaceMod]\x01 End of round has been reached.");
	#endif
	...
	while(sort){
		...
		for(int i=0; i<3; i++){
			if(score[i] < score[i+1]){
				temp1 = score[i];
				strcopy(temp2, sizeof(temp2), clientId[i]);
				score[i] = score[i+1];
				strcopy(clientId[i], sizeof(clientId[]), clientId[i+1]);
				score[i+1] = temp1;
				strcopy(clientId[i+1], sizeof(clientId[]), temp2);
				sort = true;
			}
		}
	}
			...
			if(IsClientInGame(i))
			{
				...
				{ 
					...
					if(strcmp(clientId[j], nameFromModel) == 0){
						...
						if(j == 0){
							if(client == 0){
								PrintToChatAll("\x04[RaceMod]\x01 1st place: %s with %i points.", cName, score[0]);
							}else{
								PrintToChat(client,"\x04[RaceMod]\x01 1st place: %s with %i points.", cName, score[0]);
							}
						}else if(j == 1){
							if(client == 0){
								PrintToChatAll("\x04[RaceMod]\x01 2nd place: %s with %i points.", cName, score[1]);
							}else{
								PrintToChat(client,"\x04[RaceMod]\x01 2nd place: %s with %i points.", cName, score[1]);
							}
						}else if(j == 2){
							if(client == 0){
								PrintToChatAll("\x04[RaceMod]\x01 3rd place: %s with %i points.", cName, score[2]);
							}else{
								PrintToChat(client,"\x04[RaceMod]\x01 3rd place: %s with %i points.", cName, score[2]);
							}
						}else if(j == 3){
							if(client == 0){
								PrintToChatAll("\x04[RaceMod]\x01 4th place: %s with %i points.", cName, score[3]);
							}else{
								PrintToChat(client,"\x04[RaceMod]\x01 4th place: %s with %i points.", cName, score[3]);
							}
						}
					}
				}
...
public Action Notify(Handle timer, any client) //tell players random facts
{
	if(GetConVarInt(cvar_on) > 0){
		if(msgNum == 0){
			if(finaleTips == 0){
				PrintToChatAll("\x04[RaceMod]\x01 Beat your friends to the safe room!");
			}else if(finaleTips == 1){
				PrintToChatAll("\x04[RaceMod]\x01 Kill tanks and pour gascans for points!"); //used for any other finale
			}else if(finaleTips == 2){
				PrintToChatAll("\x04[RaceMod]\x01 Be the first to the chopper!"); //used for bridge finale
			}
		}else if(msgNum == 1){
			PrintToChatAll("\x04[RaceMod]\x01 Special infected will only delay you, it's unlikely that they will kill you.");
		}else if(msgNum == 2){
			PrintToChatAll("\x04[RaceMod]\x01 Adrenaline will significantly speed you up. Use it to get that extra edge!");
		}else if(msgNum == 3){
			PrintToChatAll("\x04[RaceMod]\x01 Killing Tanks and Witches earn you bonus points!");
		}else if(msgNum == 4){
			PrintToChatAll("\x04[RaceMod]\x01 Sometimes killing special infected will give you an item! Don't forget to pick it up!");
		}else if(msgNum == 5){
			PrintToChatAll("\x04[RaceMod]\x01 Don't get incapacitated, as your friends are against you!");
		}else if(msgNum == 6){
			PrintToChatAll("\x04[RaceMod]\x01 Friendly fire is off but molotovs still hurt your friends!");
		}
...
public Action roundCountDown(Handle timer, any client) //freeze players till the counter reaches 0, then unfreeze them.
{
	if(GetConVarInt(cvar_on) > 0){
		...
		if(secondsToGo % 5 == 0 || secondsToGo < 6)
		{
			...
			{
				PrintToChatAll("\x04[RaceMod]\x01 You can't move till race starts!");			
			}
			PrintToChatAll("\x04[RaceMod]\x01 Race starts in %i seconds!", secondsToGo);
		}
		...
		if(secondsToGo < 1)
		{
			...
			PrintToChatAll("\x04[RaceMod]\x01 GO GO GO!!!");
			for(int i=1; i<=MaxClients; i++) //find everyone's client id.
			{	
				if(IsClientInGame(i))
				{
...
public Action checkPlayerPos(Handle timer, any client)
{
	if(GetConVarInt(cvar_on) > 0)
	{
		for(int player=1; player<=MaxClients; player++) //find everyone's client id.
		{
			if(IsClientInGame(player) && GetClientTeam(player) == 2) //a survivor reached the finish line
			{
				float vec[3];
